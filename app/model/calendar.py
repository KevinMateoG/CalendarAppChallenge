...
from app.services.util import date_lower_than_today_error, event_not_found_error, \
...
class Reminder:
    #EMAIL:str = "email"
    EMAIL: ClassVar[str] = "email"  #Variable de Clase
...
class Event:
    ...
    id: str = field(default_factory=app.services.util.generate_unique_id)

    def add_reminder(self, date_time: datetime, type_: str):
        new = Reminder(date_time, type_)
...
class calendar:
    ...
    def add_event(self, title: str, description: str, date_: date, start_at: time, end_at: time):
        if date_ < date(2024, 5,1): #if date_ < datetime.now().date():
